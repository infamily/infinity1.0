...
class Cryptsy:
    ...
    # User
    ##########################################################################
    #################Transfering And Withdrawing Security Info################
    ################################MUST READ#################################
    ##########################################################################
    # * In order to withdraw or transfer, you must have an Api Notification Url set in your user settings.
    # * When a request to transfer or withdraw is sent to api v2, cryptsy will do a post request to your
    # * notification url with post data including 'token', 'address', and 'amount'. Your page should return
    # * a json formatted array with the value of 'result' set to true or false which will allow or deny the
    # * transfer of coinage.
    #
    # based on: https://github.com/Webklex/CryptsyApi-v2/blob/master/Cryptsy.php#L190
    # found in: http://phptrends.com/dig_in/cryptsy
    def transfer(self, tradekey, address, quantity, notificationtoken, currencyid=id):
        return self._query(method="transfer",
                           id=currencyid,
                           query=[("notificationtoken", notificationtoken),
                                  ("tradekey", tradekey),
                                  ("address", address),
                                  ("quantity", quantity)],
                           get_method="GET")
 
    def withdraw(self, tradekey, address, quantity, notificationtoken, currencyid=id):
        return self._query(method="withdraw",
                           id=currencyid,
                           query=[("notificationtoken", notificationtoken),
                                  ("tradekey", tradekey),
                                  ("address", address),
